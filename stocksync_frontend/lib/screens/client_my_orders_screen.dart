        ...
        child: Column(
          ...
          children: [
            ...
            if (isExpanded) ...[
              ...
              // ── Assigned Staff Contact (shown when assigned or accepted) ──
              if ((status == 'assigned' || status == 'accepted') &&
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.support_agent_rounded, size: 18, color: Color(0xFF4361EE)),
                        const SizedBox(width: 10),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Assigned Staff',
                                  style: TextStyle(fontSize: 11, color: Color(0xFF6B7A9D), fontWeight: FontWeight.w600)),
                              Text(
                                o['assignedStaffName']?.toString() ?? '',
                                style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF0D1B2A)),
                              ),
                            ],
                          ),
                        ),
                        if (o['assignedStaffPhone'] != null &&
                            (o['assignedStaffPhone']?.toString() ?? '').isNotEmpty)
                          GestureDetector(
                            onTap: () async {
                              final phone = o['assignedStaffPhone'].toString();
                              final uri = Uri.parse('tel:$phone');
                              // Try to launch dialer; show phone number as fallback
                                ...
                                SnackBar(
                                  ...
                                  action: SnackBarAction(
                                    ...
                                    onPressed: () async {
                                      ...
                                      try {
                                        // Use url_launcher if available, else just show number
                                      } catch (_) {}
                                    },
                                  ),
                                ),
